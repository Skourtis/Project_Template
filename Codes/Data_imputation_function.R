...
Proteomics_imputation <- function(Data, N_condition, N_rep){
    ...
    Data <- as.data.frame(Data) %>%
        ...
        mutate(Imputted_Abundance_n = case_when(
            is.na(Abundance) & N_NA >1 & Expression == "Low" & Group_Abundance == "Single" ~TRUE,
            TRUE ~ FALSE)) %>% #If the Original Abundance is low, The replicates show the same behaviour and the expression is low , Imputes with ditribution 
        ungroup() %>%
        mutate(Imputted_Abundance_n_c = sum(Imputted_Abundance_n)) %>%
        group_by(Uniprot,Processed) %>%
        mutate(Imputted_Abundance = case_when(
            is.na(Abundance) & N_NA >1 & Expression == "Low" & Group_Abundance == "Single" ~rnorm(Imputted_Abundance_n_c,mean = mean(deciles), sd = 1),
